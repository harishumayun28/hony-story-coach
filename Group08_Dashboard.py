...
def get_recommendations(feats, shap_vals):
    """
    Produces EXACTLY 3 editorial suggestions every time.
    All possible suggestions are evaluated against actual feature values.
    They are ranked by SHAP magnitude so the most impactful come first.
    Fallback suggestions ensure we always reach 3.
    """
    ...
    # Each candidate: (shap_priority, headline, detail)
    # Lower shap_priority = more damaging = show first
    candidates = []

    # 1. Dialogue
    if dr < 0.01:
        candidates.append((
            shap_map.get("dialogue_ratio", -0.5),
            "💬 Add direct quotes from the subject",
            f"This post has zero direct quotation. HONY's most-engaged stories let the subject speak in their own words. Pull at least 2 of their actual lines into the story — it makes the person feel real to the reader."
        ))
    elif dr < 0.04:
        candidates.append((
            shap_map.get("dialogue_ratio", -0.3),
            "💬 Let them speak a little more",
            f"There's some dialogue here but more would help. Posts with strong first-person quotes from the subject earn significantly higher engagement. Consider adding one more direct line."
        ))

    # 2. Ending
    if qm == 0:
        candidates.append((
            shap_map.get("question_count", -0.4),
            "🤔 End on an open question",
            "This post closes with a statement. HONY stories that end on a question or unresolved reflection drive more reblogs and replies — they leave the reader still thinking. Rewrite the last sentence as a question."
        ))

    # 3. Word count
    if wc > 500:
        candidates.append((
            shap_map.get("word_count", -0.3),
            "📏 Tighten to under 400 words",
            f"At {wc} words this is longer than the HONY sweet spot. Posts in the 200–400 word range earn the highest engagement. Cut any sentences that repeat an idea already made — the reader gets it."
        ))
    elif wc > 400:
        candidates.append((
            shap_map.get("word_count", -0.2),
            "📏 Trim one paragraph",
            f"At {wc} words you're slightly above the high-engagement sweet spot of 200–400 words. One cut paragraph could lift this story's performance."
        ))
    elif wc < 100:
        candidates.append((
            shap_map.get("word_count", -0.2),
            "📝 Expand the narrative",
            f"At {wc} words this is very short. HONY's highest-performing posts give the subject room to breathe — aim for at least 150–200 words with more of their story."
        ))

    # 4. Sentence rhythm
    if asl > 22:
        candidates.append((
            shap_map.get("avg_sentence_length", -0.25),
    ...
    if abs(vc) < 0.3:
        candidates.append((
            shap_map.get("vader_compound", -0.2),
            "❤️ Heighten the emotional stakes",
            "This draft reads as emotionally measured. HONY's most-shared stories have a clear emotional core — vulnerability, grief, unexpected joy. Find the moment of highest feeling and make it the heart of the piece."
        ))

    # 6. First-person voice
    if fpr < 0.03:
        candidates.append((
            shap_map.get("first_person_ratio", -0.2),
            "🎤 More of their voice, less narration",
            "The post reads more as narration about the subject than as the subject's own voice. Shift more of the story into their perspective — HONY's style puts 'I' at the centre."
        ))

    # 7. Universal fallbacks — always available to fill to 3
    fallbacks = [
        (-0.05,
         "📸 Ground it in one specific detail",
        ...
        (-0.04,
         "🔤 Start with the most surprising line",
         "HONY's best posts open on the thing that made Brandon stop walking. If the most arresting moment of this story is buried in the middle, move it to the first sentence."),
        (-0.03,
         "🧍 Name the person and the place",
         "Giving the subject a first name and a specific location — even just 'outside Penn Station' or 'in Carroll Gardens' — makes the story feel grounded and real rather than universal."),
    ]

    # Sort candidates by SHAP priority (most negative = most damaging = first)
    candidates.sort(key=lambda x: x[0])

    # Fill to exactly 3 using fallbacks
...
def build_rewrite_prompt(draft, feats, recs):
    ...
    prompt = f"""You are an editorial assistant for Humans of New York (HONY), the narrative photography blog by Brandon Stanton. HONY posts are first-person, intimate, grounded in specific detail, and always centre the subject's own voice.
...
YOUR TASK:
Rewrite the story above so that it addresses every issue listed. Make real structural changes — not cosmetic rearrangement. Specifically:
- If dialogue is missing or weak, add 2 or 3 direct quotes from the subject in quotation marks, written in their authentic voice, consistent with the story
...
- Keep Brandon Stanton's editorial voice: short paragraphs, intimate, grounded in specific people and places

YOUR RESPONSE MUST CONTAIN EXACTLY THESE TWO SECTIONS WITH THESE EXACT HEADERS:

REWRITTEN STORY:
[the full rewritten story here — nothing else in this section]

WHAT I CHANGED:
[exactly 3 bullet points starting with a dash, each explaining one specific structural change you made and why it improves the story]"""

    return prompt

# ── Parse Claude response ─────────────────────────────────────────────────────
def parse_response(response_text):
    """Robustly split Claude response into rewrite + changes."""
    # Try the expected format first
    if "WHAT I CHANGED:" in response_text:
        parts   = response_text.split("WHAT I CHANGED:", 1)
        rewrite = parts[0].replace("REWRITTEN STORY:", "").strip()
        changes_raw = parts[1].strip()
    elif "What I Changed:" in response_text:
        parts   = response_text.split("What I Changed:", 1)
        rewrite = parts[0].replace("REWRITTEN STORY:", "").replace("Rewritten Story:", "").strip()
        changes_raw = parts[1].strip()
    else:
        # Fallback: treat whole response as rewrite, generate generic changes note
        rewrite = response_text.strip()
        changes_raw = "- Story rewritten to address editorial suggestions above"
    ...
    return rewrite, changes[:5]  # cap at 5 change items
...
# ── Run analysis and save to session state ────────────────────────────────────
if analyse:
    if len(draft_input.strip()) < 30:
        ...
        st.session_state["results"] = dict(
            ...
            topic_label=topic_label, tags=tags, recs=recs,
            draft=draft_input
        )
        # Clear previous rewrite when new analysis runs
...
if "results" in st.session_state:
    ...
    tags        = R["tags"]
    recs        = R["recs"]          # guaranteed list of 3 tuples
    draft       = R["draft"]
    ...
    for i, (headline, detail) in enumerate(recs):
        st.markdown(f"""
        <div class="rec-card">
    ...
    st.markdown(
        ...
        'Click below to apply all three suggestions automatically. '
        'The rewrite will show exactly what changed and why — '
        'review and edit in your own voice before publishing.</div>',
        unsafe_allow_html=True)
        ...
        with st.spinner("Rewriting..."):
            try:
                ...
                if not api_key:
                    ...
                    rewrite, changes = parse_response(raw)
                    st.session_state["rewrite"] = rewrite
                    st.session_state["changes"] = changes
            except ImportError:
                st.error("anthropic package not installed. Add `anthropic` to requirements.txt and redeploy.")
            except Exception as e:
                st.error(f"Rewrite unavailable: {e}")

    # Show rewrite side by side with word counts
    if "rewrite" in st.session_state:
        ...
        if changes:
            ...
            for headline, detail in recs:
                clean_h = headline.split(" ", 1)[1] if headline[0] in "📏💬🎤❤️🤔✍️📝📸🔤🧍" else headline
                st.markdown(f'<div class="change-card">✓ {clean_h} — applied to the rewrite above</div>', unsafe_allow_html=True)
    ...
    with st.expander("See the data behind this prediction"):
        ...
        st.markdown(
            ...
            'XGBoost trained on 12 content features. '
            'Temporal features excluded to avoid era bias. '
            'Cross-validated ROC AUC: 0.85. Does not model the photograph.</div>',
            unsafe_allow_html=True)
