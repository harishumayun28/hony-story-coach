        ...
        if fname == "word_count":
            if fval > 450:
                recs.append(("📏 Consider shortening this story",
                    f"Stories under 400 words earn significantly more engagement. Yours is {fval:.0f} words — try trimming the middle."))
            elif fval < 100:
                recs.append(("📝 This story could use more depth",
                    f"Very short posts miss the narrative richness HONY readers come for. Yours is {fval:.0f} words."))
        elif fname == "dialogue_ratio" and fval < 0.03:
            recs.append(("💬 Add more of their exact words",
                "Posts with strong first-person dialogue earn higher engagement. Let them speak more directly."))
        elif fname == "first_person_ratio" and fval < 0.04:
            recs.append(("🎤 Let the subject's voice lead",
                "HONY's signature is raw first-person voice. More 'I' and 'my' language draws the reader in."))
        elif fname == "vader_compound" and abs(fval) < 0.3:
            recs.append(("❤️ Find the emotional core",
                "This draft reads as fairly neutral. Posts with a clear emotional undercurrent consistently outperform."))
        elif fname == "question_count" and fval == 0:
            recs.append(("🤔 End on an open moment",
                "A reflective question or unresolved thought at the end drives more reblogs."))
        elif fname == "avg_sentence_length" and fval > 25:
            recs.append(("✍️ Vary the sentence rhythm",
                f"Average sentence length is {fval:.0f} words. Short punchy sentences mixed with longer ones give HONY stories their pace."))
        if len(recs) >= 3:
            break
    return recs
...
# ── Save results to session state on analyse ──────────────────────────────────
if analyse:
...
if "results" in st.session_state:
    ...
    st.markdown('<div class="rec-detail" style="margin-bottom:.9rem;">Apply these suggestions automatically. Review and edit before publishing — this is a starting point, not a final draft.</div>', unsafe_allow_html=True)

    if st.button("Rewrite for better engagement", type="secondary"):
        rec_text = "\n".join([f"- {h}: {d}" for h, d in recs]) if recs else "No major issues found."
        prompt = f"""You are an editorial assistant for Humans of New York (HONY).

Draft story:
{draft}

Issues found by our engagement model:
{rec_text}

Rewrite this story applying the suggestions. Keep Brandon Stanton's voice — first person, conversational, intimate, grounded in specific detail. Do not invent facts. Keep the same subject and story. Aim for under 400 words if the original is longer. If there are no direct quotes, add 1-2 that feel consistent with the story.
        ...
        with st.spinner("Rewriting..."):
            try:
                ...
                if not api_key:
                    ...
                    st.session_state["rewrite"] = message.content[0].text
            except ImportError:
    ...
    if "rewrite" in st.session_state:
        ...
        with c1:
            ...
            st.text_area("", value=st.session_state["results"]["draft"],
                         height=300, disabled=True, key="orig_txt")
        with c2:
